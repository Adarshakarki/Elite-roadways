...
function Home() {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-b from-[#F9F6E6] to-[#BAD8B6] px-4">
      <h2 className="text-3xl font-bold text-[#8D77AB] mb-6">Search for Buses</h2>
      <form onSubmit={handleSearch} className="flex flex-col w-full max-w-sm space-y-4 bg-white bg-opacity-80 p-6 rounded-xl shadow-lg">
        <input
          ...
          className="px-4 py-3 rounded-md border border-gray-300 focus:outline-none focus:ring-2 focus:ring-[#8D77AB]"
        />
        <button
          type="submit"
      ...
      {/* Conditionally render the "Add Bus" button for admin */}
      {
        <button
          ...
          className="mt-6 px-6 py-3 bg-[#859F3D] text-white font-medium rounded-md shadow-lg hover:bg-[#31511E] transition duration-300 ease-in-out"
        >
          Add Bus
        </button>
      }
